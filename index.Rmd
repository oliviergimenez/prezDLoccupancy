...
+ Pour mes analyses, j'ai utilisé la librairie fastai qui repose sur le langage Python et sa librairie Pytorch. Fastai simplifie l'entrainement des réseaux de neurones avec l'utilisation des méthodes les plus récentes. 
...
+ Le gros avantage de fastai est qu'elle vient aussi avec un package R qui propose tout un tas de fonctions pour l'utiliser. 
...
+ On charge le package fastai d'abord. 
...
+ Puis on continue l'entrainement sur nos données. Ici j'ai utilisé 20 epochs et sauvegardé le modèle après chaque epoch pour une utilisation ultérieure. 
...
+ Les résultats peuvent être représentés sous la forme d'une matrice de confusion, avec les espèces qui se trouvent effectivement sur les photos en lignes, et les espèces qui sont prédites par l'algo en colonnes.  

+ On se concentre sur le lynx et ses proies chevreuil et chamois et on dissèque les faux positifs et faux négatifs.  

+ Un faux négatif est par ex une photo sur laquelle on a un lynx mais on prédit autre chose. Un faux positif est une photo sur laquelle on n'a pas de lynx mais on prédit qu'il y en a un. 

+ On commence avec les faux négatifs, en ligne donc :
    + le lynx est confondu avec chat et renard surtout ;
    + chamois avec chevreuil parfois ;
    + chevreuil avec chamois, chien et renard.

+ On continue avec les faux positifs, les colonnes donc :
...
+ C'est pas jojo. Il y a un peu de frustration, il ne faut pas le cacher... Mais passée cette frustration, on s'est posés la question, est-ce bien grave par rapport à la question écologique des interactions prédateur-proie?
...
+ Pour voir si les biais sont grave, on a utilisé des modèles statistiques qui permettent d'inférer les co-occurrences entre espèces en tenant compte de la difficulté de les détecter sur le terrain, les modèles dits d'occupancy. 
...
+ Ces modèles sont relativement faciles à implémenter dans R avec le package unmarked. 
...
+ A partir de ces données, il est relativement simple de construire des modèles pour tester nos hypothèses sur les interactions. 

+ Ici je ne regarde que les interactions possibles entre le lynx et ses proies chamois et chevreuil, tout en prenant également en compte la présence de chats et renards. 
...
+ Maintenant quand on passe aux probabilités de présence du prédateur, conditionnellement à la présence ou absence de ses proies, c'est une autre histoire. 

+ Par exemple, la prob de présence du lynx sachant la présence du chevreuil et l'absence du chamois est clairement sous-estimée.
...
+ On vérifie ici que l'utilisation d'un modèle entrainé sur un site, ici le Jura, est délicate quand il s'agit de prédire sur un autre site, ici l'Ain. C'est un résultat connu. 
...
+ Il est facile de se perdre dans les dédales du deep learning, mais il faut garder le cap de la question écologique, et on peut peut-être accepter des performances moyennes si on peut vivre avec le biais engendré sur les estimations des indicateurs écologiques que l'on souhaite utiliser. 
...
+ On peut toutefois sûrement faire mieux, et j'explore actuellement le développement de modèles de distribution d'espèce qui prendrait à la fois en compte les interactions et les faux positifs et faux négatifs. 
